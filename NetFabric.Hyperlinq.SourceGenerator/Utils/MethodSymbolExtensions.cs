...
namespace NetFabric.Hyperlinq.SourceGenerator
{
    struct MethodInfo
    {
        ...
        public IReadOnlyList<(string Name, string Constraints)> TypeParameters { get; set; }
        ...
        public static MethodInfo GetInfo(this IMethodSymbol method, int skip = default)
            => new MethodInfo
            {
                ...
                TypeParameters =
                    ...
                    .Select(parameter => (parameter.ToDisplayString(), parameter.AsConstraintsStrings().ToCommaSeparated()))
                    .ToArray(),
            };

        public static MethodInfo ApplyMappings(this MethodInfo method, ImmutableArray<(string, string, bool)> genericsMapping) 
            => genericsMapping.IsDefault
                ? method
                : new MethodInfo
                    {
                        ContainingType = method.ContainingType.ApplyMappings(genericsMapping),
                        ReturnType = method.ReturnType.ApplyMappings(genericsMapping),
                        Name = method.Name,
                        Parameters = method.Parameters
                            .Select(parameter => (parameter.Name, parameter.Type.ApplyMappings(genericsMapping), parameter.DefaultValue))
                            .ToArray(),
                        TypeParameters = method.TypeParameters
                            .Select(parameter => (parameter.Name.ApplyMappings(genericsMapping), parameter.Constraints.ApplyMappings(genericsMapping)))
                            .ToArray(),
                    };
